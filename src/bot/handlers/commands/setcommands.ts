...
export async function setCommandsHandler(ctx: CommandContext<Context>) {
  const start = new Command('start', i18n.t('ru', 'start.description'))
    .addToScope({ type: 'all_private_chats' })
  addCommandLocalizations(start)
  addCommandToChats(start, ctx.config.botAdmins)

  const language = new Command('language', i18n.t('ru', 'language.description'))
    .addToScope({ type: 'all_private_chats' })
  addCommandLocalizations(language)
  addCommandToChats(language, ctx.config.botAdmins)

  const setcommands = new Command('setcommands', i18n.t('ru', 'setcommands.description'))
