...
/**
 * Результат валидации формы с обязательной ошибкой
 */
export type FormValidateResultWithReason<T, R> = { ok: false, error: R } | { ok: true, value: T }
...
type FormBuilderLikeWithReason<T, R> = {
  validate: (ctx: Context) => Promise<FormValidateResultWithReason<T, R>>
...
export type BuildOptionalFieldOptions = {
  ...
  /** Сообщение при пропуске поля (опционально) */
  skipMessage?: string
